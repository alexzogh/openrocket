...
* **3D Flight Animation** (Tools > Flight Animation): Replay completed simulations as an animated 2D trajectory visualization. Features a velocity-colored flight path, flight event markers (launch, burnout, apogee, deployment, landing), play/pause controls, adjustable playback speed (0.25x to 8x), timeline scrubbing, and real-time data readouts for altitude, velocity, Mach number, and acceleration.

* **Motor Recommendation Engine** (Tools > Recommend Motors...): Automatically analyzes your rocket design and recommends suitable motors from the database. Filters by physical fit (diameter and length), computes thrust-to-weight ratio, estimates apogee altitude, and provides a color-coded safety rating (green/yellow/red). Supports filtering by motor type (single-use, reloadable, hybrid).

* **Ejection Charge Calculator** (Tools > Ejection Charge Calculator...): Calculates the black powder (FFFFg BP) ejection charge needed for dual-deployment recovery systems. Auto-populates airframe dimensions from your rocket design, supports adjustable target pressure (5-20 PSI), and displays results in grams, ounces, and teaspoons. Includes metric/imperial unit toggle and safety warnings.

* **Ring-Tail Fin Component** (Component Palette > Ring-tail): A new annular (ring-shaped) fin component that wraps around the rocket body as a cylindrical band. Configurable ring radius, chord length, and wall thickness. Attachable to body tubes and transitions. Includes full mass, CG, and inertia calculations.

* **STL Export for 3D Printing** (File > Export As > STL): Export rocket components in binary STL format, the universal standard for 3D printing. Reuses the existing OBJ geometry pipeline for accurate mesh generation. Includes export options for component selection, level of detail, scaling (defaults to millimeters), and child component inclusion.
